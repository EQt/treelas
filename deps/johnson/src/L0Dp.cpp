...
{
  public:
    Segmenter(const RealVec &o) : obs_(o) { this->CalcCumsums(); }
    ...
    void Segment(int n_seg, IntVec *seg_bds)
    {
        ...
        for (int i = 1; i < n_seg; ++i) {
            for (int j = 0; j < nobs; ++j) {
                ...
                for (int k = 0; k < j; ++k) {
                    double new_val =
                        this->GetSegmentLik(k + 1, j) + dp_mat[i - 1][k].second;
                    if (new_val > best_val) {
                        best_val = new_val;
                        best_idx = k;
                    }
  ...
  protected:
    double get_cumsum(int i) const { return cumsums_[i]; }
...
SEXP
L0DpSegment(SEXP o, SEXP lik_type, SEXP n_seg)
{
    using namespace Util;
