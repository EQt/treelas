...
L0GaussByNseg = function(x, nSegs = 3, obsWts = NULL, maxSegs = 3000,
                         avgNumBackptrSegs = 100) {
  n = length(x)
  ...
  z = rep(0.0, n)
  .Call("EfamL0VitByNseg", vitTerms, nSegs, z, maxSegs,
			  avgNumBackptrSegs)
  return(z)
}

L0Gauss = function(x, lambda2 = NULL, obsWts = NULL,
                   maxSegs = 3000, avgNumBackptrSegs = 100) {
  n = length(x)
  vitTerms = matrix(nrow = 2, ncol = n, data = -1.0)
  vitTerms[1, ] = 2.0 * x
  ...
  z = rep(0.0, n)
  .Call("EfamL0Vit", vitTerms, lambda2, z, maxSegs,
        avgNumBackptrSegs)
  return(z)
}
